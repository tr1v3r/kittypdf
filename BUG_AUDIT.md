...
- **真 kitty 冒烟 ✅已完成（2026-09-20，kitty 0.48.2 真机 + remote-control 自动化）**：①40 页测试 PDF 显示正常（自动 spread 双页，Quartz `CGWindowListCreateImage` 直捕窗口 + OCR 确认两页图像与页码文字）；②键位全验：`2e`/`2u` count 步进、`t` overlay 列章 + Enter 跳章、`G` 末页、`i` 反色、`a` 透明、`3E`/`5u` 大跳——状态栏页号逐一精确（q=2 生效，无幻影按键）；③`q` 干净退出 exit 0，进度缓存正确落盘（page/invert）。驱动方式：`kitty --override allow_remote_control=yes --listen-on unix:…` + `kitty @ send-text` / `get-text`；窗口内容验证必须用 Quartz 直捕——屏幕合成截图（screencapture）会被窗口背景透明度污染。macOS 沙箱注意：kitty 无视 `XDG_CACHE_HOME`、必须写 `~/Library/Caches/kitty`，受限环境需放行。原②的 pty 模拟器路径未复跑（/tmp/kitty_emu3.py 已不存在；真机覆盖更全，不再需要）。
